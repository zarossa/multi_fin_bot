...
async def train_all(message: types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        data['pk'] = res.get('pk')
        await message.reply(f'{data["step"]}. Word is { data["word"] }', reply_markup=inline_kb)
...
async def button_click_call_back_all(callback_query: types.CallbackQuery, state: FSMContext):
    ...
    async with state.proxy() as data:
        if answer == data.get('answer'):
            await bot.send_message(callback_query.from_user.id, f'Yes!\n')
            ...
            if res:
                ...
                data['word'] = res.get('word')
                data['pk'] = res.get('pk')
                await bot.send_message(callback_query.from_user.id, f'{data["step"]}. Word is { data["word"] }',
